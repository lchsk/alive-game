...
void render_engine(Engine* engine) {
    ...
    if (engine->state == ENGINE_LOADING) {
        if (engine->loader->total_to_load > 0) {
            char text[100];
            sprintf(text, "Alive: the game | Loading: %d%%",
                    (int)ceil(engine->loader->total_loaded /
                              (float)engine->loader->total_to_load * 100));
...
void handle_input(Engine* engine) {
    if (engine->input->mouse_left_down ||
        (player_state.weapon == WEAPON_MACHINE &&
         engine->input->mouse_left_pressed)) {
        ...
        if ((player_state.game_state == GAME_STATE_PLAY ||
            ...
            e && player_state.since_last_shot > time_required) {
            player_state.since_last_shot = 0;
            ...
            if (player_state.weapon == WEAPON_PISTOL) {
                int channel = Mix_PlayChannel(
                    -1, engine->loader->sound_loader->sounds[SOUND_PISTOL], 0);
                Mix_Volume(channel, MIX_MAX_VOLUME * SOUND_VOLUME);

                bp->speed_factor = PISTOL_BULLET_SPEED;
            } else if (player_state.weapon == WEAPON_RIFLE) {
                int channel = Mix_PlayChannel(
                    -1, engine->loader->sound_loader->sounds[SOUND_RIFLE], 0);
                Mix_Volume(channel, MIX_MAX_VOLUME * SOUND_VOLUME);

                bp->speed_factor = RIFLE_BULLET_SPEED;
            } else if (player_state.weapon == WEAPON_MACHINE) {
                int channel = Mix_PlayChannel(
                    -1, engine->loader->sound_loader->sounds[SOUND_MACHINE], 0);
                Mix_Volume(channel, MIX_MAX_VOLUME * SOUND_VOLUME);
    ...
    for (int i = 0; i < BULLETS_COUNT; i++) {
        ...
        if (bp->is_enemy) {
            if (entity_has_collision(bullet, &player->rects[0])) {
                ...
                bp->is_flying = 0;
                player_state.health -=
                    get_rand_int(PLAYER_DAMAGE_MIN, PLAYER_DAMAGE_MAX);
        ...
        for (int j = 0; j < ENEMIES_COUNT; j++) {
            ...
            if (entity_has_collision(enemy, &bullet->rects[0])) {
                ...
                if (bp->strength == WEAPON_PISTOL) {
                    ...
                    damage =
                        get_rand_int(MACHINE_DAMAGE_MIN, MACHINE_DAMAGE_MAX);
                }
...
void render_hud(Engine* engine) {
    ...
    text_gun->sprite->position.x =
        engine->width - text_gun->sprite->texture->width - 20;
...
void setup_loading_screen() {
    ...
    text_loading->sprite->position.x =
        engine->width / 2 - text_loading->sprite->texture->width / 2;
    text_loading->sprite->position.y =
        engine->height / 2 - text_loading->sprite->texture->height / 2;
...
void setup_game() {
    ...
    cursor =
        sprite_new(engine->loader->texture_loader->textures[TEX_CROSSHAIR]);

    level = build_level_1(engine);

    player = entity_new();
    Sprite* sprite =
        sprite_new(engine->loader->texture_loader->textures[SURVIVOR_GUN]);
    ...
    for (int i = 0; i < BULLETS_COUNT; i++) {
        Sprite* sprite =
            sprite_new(engine->loader->texture_loader->textures[TEX_BULLET]);
    ...
    text_set(text_game_end, engine->renderer,
             "You lost. Play again? (yes - space/no - q)");
    text_game_end->sprite->position.x =
        engine->width / 2 - text_game_end->sprite->texture->width / 2;
    text_game_end->sprite->position.y =
        engine->height / 2 - text_game_end->sprite->texture->height / 2;
    ...
    text_set(text_help[2], engine->renderer,
             "Mouse - change direction/target, Mouse left button - shoot");
    ...
    text_set(text_help[4], engine->renderer,
             "Press space to start, esc to quit");
    text_help[4]->sprite->position.x = 300;
    text_help[4]->sprite->position.y = 400;
    text_set(text_help[5], engine->renderer,
             "Visit https://lchsk.com and https://github.com/lchsk");
...
int main() {
    ...
    Loader* loader =
        loader_new(TEXTURES_COUNT, FONTS_COUNT, MUSIC_COUNT, SOUNDS_COUNT);
