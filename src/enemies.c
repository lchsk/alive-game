...
void init_enemies(Engine *engine) {
    for (int i = 0; i < ENEMIES_COUNT; i++) {
        ...
        Sprite *sprite = sprite_new(engine->loader->texture_loader->textures[texture]);
        e->sprite = sprite;
        e->properties = &ep[i];
    }
}
        ...
        for (int i = 0; i < ENEMIES_COUNT; i++) {
            ...
            if (ep->is_active) {
                ...
                if (get_distance(p1, p2) > PLAYER_ENEMY_MIN_DISTANCE) {
                    entity_set_position(e, e->position.x + ep->speed_x * engine->_delta, e->position.y + ep->speed_y * engine->_delta);
                }
...
void enemies_update(Engine *engine, Entity *player, PlayerState *player_state, GameTimer *timer) {
    ...
    if (timer->since_last_enemy_launched > launch) {
        ...
        if (enemy) {
            ...
            if (get_rand_int(0, 2) == 0) {
                ...
                initial_y = get_rand_int(1.1 * engine->height, 1.2 * engine->height);
            }
    ...
    for (int i = 0; i < ENEMIES_COUNT; i++) {
        ...
        if (ep->is_active) {
            ...
            // Shoot?
            int shot_interval = get_rand_int(ENEMY_SHOT_INTERVAL_MS, ENEMY_SHOT_INTERVAL_MS + ENEMY_SHOT_INTERVAL_RANGE);
            ...
            if (ep->since_last_shot > shot_interval) {
                ...
                if (bullet) {
                    ...
                    Pointd vec = get_pos_entities((Pointd){player->position.x, player->position.y}, e);
